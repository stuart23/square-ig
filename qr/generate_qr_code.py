...
def mask_image_and_outline(input_image, outline=True, outline_width=7, outline_color=(0,0,0)):
    blank = Image.new(
        input_image.mode, 
        input_image.size, 
        (255,255,255)
...
def __main__(filename, code_text):
    qr = qrcode.QRCode(error_correction=qrcode.ERROR_CORRECT_H, box_size=10, border=0)
    ...
    qr_code_image = qr.make_image(
        ...
        color_mask=SolidFillColorMask(back_color=dark_green, front_color=light_green)
    )._img

    masked_image = mask_image_and_outline(qr_code_image, outline=2)
    output_filename_color = 'qr_{}_color.png'.format(filename)
    masked_image.save(output_filename_color)
    ...
    masked_image = mask_image_and_outline(qr_code_image, outline=2)
    output_filename_bw = 'qr_{}_bw.png'.format(filename)
    masked_image.save(output_filename_bw)
    output = {'color filename': output_filename_color, 'bw filename': output_filename_bw}

if __name__ == "__main__":
    __main__(filename='asdf1234', code_text='plantsoc.app/asdf1234')