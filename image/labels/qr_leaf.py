...
class QRLeaf(object):
    ...
    def colour_qr(self):
        ...
        qr_code_image = self.qr.make_image(
            ...
            color_mask=SolidFillColorMask(
                back_color=self.dark_green, front_color=self.light_green
            ),
        )._img
    ...
    def _random_pad(qr):
        ...
        for row_index in range(new_size):
            for col_index in range(new_size):
                qr_row_coordinate, qr_column_coordinate = [
                    row_index - random_padding - 1,
                    col_index - random_padding - 1,
                ]
                if all(
                    [
                        qr_row_coordinate >= 0,
                    ...
                    output[row_index][col_index] = qr.modules[qr_row_coordinate][
                        qr_column_coordinate
                    ]
                elif all(
                    [
    ...
    @staticmethod
    def _mask_image_and_outline(
        input_image, outline=True, outline_width=7, outline_color=(0, 0, 0)
    ):
        blank = Image.new(input_image.mode, input_image.size, (255, 255, 255))
        mask = Image.open(MASK).resize(input_image.size)
        masked_image = Image.composite(input_image, blank, mask)
        if outline:
            outline_image = mask.filter(ImageFilter.FIND_EDGES).filter(
                ImageFilter.MaxFilter(outline_width)
            )
            blank_black = Image.new(masked_image.mode, masked_image.size, outline_color)
            return Image.composite(
                masked_image, blank_black, ImageOps.invert(outline_image)
            )
        else:
            return masked_image
