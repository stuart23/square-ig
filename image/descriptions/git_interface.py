...
class DescriptionsGit(object):
    def __init__(self, repo_dir=REPO_DIR):
        ...
        self.git_environment = {
            "GIT_SSH_COMMAND": f"ssh -o StrictHostKeyChecking=no -i {KEY_FILE}"
        }
        self.repo_dir = Path(REPO_DIR)
        self.repo_dir.mkdir()

        self.repo = Repo.clone_from(
            self.repo_url, self.repo_dir, env=self.git_environment
        )
    ...
    def add_item(self, item, replace=False):
        ...
        if item_dir.is_dir() and not replace:
            ...
            print(
                f"Deleting and recreating for {item} as directory {item_dir} already exists"
            )
            rmtree(item_dir)
        item_dir.mkdir()
