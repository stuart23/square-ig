...
def get_website_needs_update_items():
    ...
    response = table.query(
        IndexName="websiteIndex", KeyConditionExpression=Key("website").eq("N")
    )
...
def get_needs_label_items():
    ...
    """
    response = table.query(
        IndexName="labelIndex", KeyConditionExpression=Key("label").eq("N")
    )
    count = response["Count"]
...
def get_item_by_sku(sku):
    ...
    """
    response = table.query(
        IndexName="skuIndex", KeyConditionExpression=Key("SKU").eq(sku)
    )
